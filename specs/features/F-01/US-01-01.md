...
**As a** operator opening the application for the first time, **I want to** see a branded login screen with the product name, tagline, and a clear demo mode notice, **so that** I immediately understand what product I am using and do not mistake the PoC for a production system.
...
|----|-----|-----------|------|
| TC-01-01-01 | AC-1 | Verify the login screen is the only visible view on initial page load | E2E |
| TC-01-01-02 | AC-2 | Verify the lightning bolt logo mark is rendered in the top-centre of the card | E2E |
| TC-01-01-03 | AC-3 | Verify the product name "BESS Intelligence" is displayed adjacent to the logo | E2E |
| TC-01-01-04 | AC-4 | Verify the tagline text is visible beneath the logo and name | E2E |
| TC-01-01-05 | AC-5 | Verify the login card is centred horizontally and vertically on viewports ≥ 768 px | E2E |
| TC-01-01-06 | AC-6 | Verify the layout is not broken on 1024 px (laptop) and 768 px (tablet) viewports | E2E |
| TC-01-01-07 | AC-7 | Verify the demo mode notice text is visible below the "Sign in" button | E2E |
| TC-01-01-08 | AC-8 | Verify the demo note uses a smaller font and muted colour | E2E |
| TC-01-01-09 | AC-9 | Verify a "Forgot password?" link is visible below the "Sign in" button | E2E |
| TC-01-01-10 | AC-10 | Verify clicking "Forgot password?" produces no navigation and no error | E2E |
