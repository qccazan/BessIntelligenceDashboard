...
**As a** operator who switches between assets to compare their daily behaviour, **I want to** the replay chart and animation to reset and restart automatically when I select a different asset, **so that** I always see a fresh replay starting from the beginning of the day for whichever asset I am currently inspecting.
...
|----|-----|-----------|------|
| TC-06-06-01 | AC-1 | Verify selecting a different asset regenerates the power bars and SoC curve | E2E |
| TC-06-06-02 | AC-2 | Verify the playhead resets to the leftmost position after asset switch | E2E |
| TC-06-06-03 | AC-3 | Verify the animation starts playing automatically after the chart regenerates | E2E |
| TC-06-06-04 | AC-4 | Verify the timestamp readout resets to the start time after asset switch | E2E |
| TC-06-06-05 | AC-5 | Verify the daily energy summary chips recalculate for the newly selected asset | E2E |
| TC-06-06-06 | AC-6 | Verify the end-of-day SoC in the replay is consistent with the current SoC in the Current State card | E2E |
